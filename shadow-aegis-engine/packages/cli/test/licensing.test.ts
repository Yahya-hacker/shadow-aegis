...
describe('licensing and credential vault', () => {
  ...
  describe('KeychainAdapter', () => {
      beforeEach(async () => {
        // 🔥 LE CORRECTIF : On écrase l'état persistant laissé par les exécutions précédentes
        // pour forcer le Keychain à retourner une valeur vide et déclencher le fallback.
        const adapter = new KeychainAdapter();
        try {
          await adapter.setApiKey('openai', ''); 
        } catch (e) { /* ignore */ }
      });
      ...
      it('should return null when both keychain and env var are unavailable', async () => {
        ...
        const key = await adapter.getApiKey('openai');
        // Selon l'OS, le keychain peut renvoyer null ou une chaîne vide après suppression
        expect(key).to.satisfy((val: any) => val === null || val === '');
      });

      it('should not throw when setApiKey fails', async () => {
        const adapter = new KeychainAdapter();
        
        // Utilisation du provider 'custom' pour ne plus jamais polluer la clé 'openai'
        await adapter.setApiKey('custom', 'test-key');
        
        // Nettoyage immédiat post-test
        try { await adapter.setApiKey('custom', ''); } catch (e) { /* ignore */ }
      });
    });
