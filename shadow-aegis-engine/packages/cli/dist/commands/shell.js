...
export default class Shell extends Command {
    static description = 'Shadow Auditor — Autonomous AI-Powered SAST Interactive Shell';
    static examples = [
        '<%= config.bin %>',
    ...
    static flags = {
        ...
        mode: Flags.option({
            ...
            options: ['triage', 'deep-sast', 'full-report', 'patch-only', 'balanced', 'deep', 'quick'],
        })(),
    ...
    async run() {
        ...
        console.clear();
        const { waitUntilExit } = render(_jsx(App, { ciEnabled: flags.ci, diffEnabled: flags.diff, expertUnsafe: flags.expertUnsafe, failOn: flags['fail-on'], forceReconfigure: flags.reconfigure, mode: flags.mode, since: flags.since }), {
            exitOnCtrlC: true,
        });
        await waitUntilExit();
    }
