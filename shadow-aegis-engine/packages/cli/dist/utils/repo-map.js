...
function extractSignature(node, sourceCode) {
    ...
    if (node.type === 'export_statement') {
        const declaration = node.namedChildren.find((c) => c.type === 'function_declaration' ||
 ...
 */
async function parseFile(parser, filePath, basePath) {
    ...
    try {
        ...
        for (const child of root.namedChildren) {
            if (STRUCTURAL_TYPES.has(child.type) ||
        ...
        const relativePath = path.relative(basePath, filePath);
        console.warn(`⚠  Tree-sitter parse error in ${relativePath}: ${error.message}`);
        return null;
    }
}
/**
 ...
 */
export async function generateRepoMap(targetPath) {
