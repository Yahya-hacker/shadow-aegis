 ...
 */
const calculateNextFrame = (currentIndex, totalFrames, loop) => {
...
export const AsciiMotionCli = ({ autoPlay = true, hasDarkBackground = true, loop = DEFAULT_LOOP, onReady, }) => {
    ...
    const getColor = useCallback((key) => colors[key] || key, [colors]);
    ...
    return (_jsx(Box, { flexDirection: "column", children: frame.content.map((row, y) => (_jsx(Box, { children: row.split("").map((char, x) => {
                const posKey = `${x},${y}`;
                ...
                return (_jsx(Text, { backgroundColor: bg, color: fg, children: char }, x));
            }) }, y))) }));
};
