...
export function evaluateMcpPolicy(adapterId, toolDefinition, expertUnsafe) {
    const safeTools = SAFE_ADAPTER_TOOLS[adapterId] ?? new Set();
    ...
    if (isHighRisk) {
        return {
            ...
            warning: '[EXPERT-UNSAFE] High-risk MCP operation requested. Confirm only when you are authorized and the blast radius is understood.',
        };
    }
    return {
        allowed: true,
