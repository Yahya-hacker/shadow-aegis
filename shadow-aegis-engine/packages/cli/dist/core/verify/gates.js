 ...
 */
import { calculateConfidence, CONFIDENCE_THRESHOLDS, } from './confidence.js';
...
export class VerificationGates {
     ...
     */
    hasMinimumEvidence(candidate) {
        return Boolean((candidate.entityIds && candidate.entityIds.length > 0) ||
    ...
    verify(candidate) {
        ...
        const linking = this.evidenceLinker.linkFinding(candidate.title, candidate.entityIds ?? [], candidate.toolRunRefs ?? []);
    ...
    // Individual Gates
    // ==========================================================================
    gateAssumptionsFlagged(candidate) {
    ...
    gateCodeEvidencePresent(candidate) {
    ...
    gateDataFlowPresent(candidate) {
        ...
        const flowResult = this.evidenceLinker.verifyDataFlowEvidence(candidate.sourceId, candidate.sinkId, candidate.relatedEntityIds ?? []);
        return {
    ...
    gateNoContradictions(candidate) {
        const checkResult = this.contradictionChecker.checkFinding(candidate.title, candidate.sourceId, candidate.sinkId, [...(candidate.entityIds ?? []), ...(candidate.relatedEntityIds ?? [])]);
    ...
    gateTruncationFlagged(candidate) {
