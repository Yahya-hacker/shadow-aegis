 ...
 */
export function cweToCategory(cwe) {
...
export class ChainBuilder {
    ...
    retrieval;
    stepManager;
    constructor(graph, retrieval, options = {}) {
        this.graph = graph;
     ...
     */
    async buildChainFromVulnerability(vuln) {
        ...
        // Create the chain
        const chainResult = this.chainManager.createChain(`${category.toUpperCase()} Attack Chain: ${props.title}`, `Attack chain exploiting ${cwe}: ${props.title}`, stepIds);
        if (!chainResult.ok) {
    ...
    async buildFromDataFlows() {
        ...
        for (const source of sources) {
            for (const sink of sinks) {
                ...
                if (bestPath.confidence < this.options.minConfidence)
                    continue;
     ...
     */
    async buildFromVulnerabilities() {
     ...
     */
    async buildChainFromDataFlow(source, sink, path) {
        ...
        // Create the chain
        const chainResult = this.chainManager.createChain(`Data Flow Attack: ${sourceProps.name} → ${sinkProps.name}`, `Taint flow from ${sourceProps.category} source to ${sinkProps.category} sink`, stepIds);
     ...
     */
    getCweForSinkCategory(sinkCategory) {
        switch (sinkCategory) {
            case 'dom': {
                return 'CWE-79';
            }
     ...
     */
    sinkCategoryToAttackCategory(sinkCategory) {
        switch (sinkCategory) {
            case 'dom': {
                return 'xss';
            }
