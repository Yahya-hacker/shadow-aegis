 ...
 */
function extractFileContext(root) {
    const contextLines = [];
 ...
 */
function extractClassHeader(node) {
    const name = node.childForFieldName('name')?.text ?? 'Anonymous';
    const superClass = node.childForFieldName('superclass');
    let header = `class ${name}`;
    if (superClass)
        header += ` extends ${superClass.text}`;
    return header;
}
/**
 * Chunk a parsed AST into semantically meaningful code blocks.
 */
function chunkAST(root, sourceCode, filePath, language, maxChunkChars) {
    const chunks = [];
    const fileContext = extractFileContext(root);
    const lines = sourceCode.split('\n');
    function createChunk(node, structuralType, symbol, parentContext) {
    ...
    for (const child of root.namedChildren) {
            ...
            if (body) {
                ...
                for (const member of body.namedChildren) {
                    if (member.type === 'method_definition') {
                        ...
                        chunks.push(createChunk(member, 'method', `${className}.${methodName}`, classContext));
        ...
        if (child.type === 'export_statement') {
            const declaration = child.namedChildren.find((c) => c.type === 'function_declaration' ||
...
export class SemanticIndex {
    ...
    async indexFile(filePath) {
        ...
        try {
            ...
            console.warn(`[SemanticIndex] Parse error in ${filePath}: ${error.message}`);
            return 0;
        }
    }
    /**
     ...
     */
    async indexRepository(onProgress) {
        this.ensureInitialized();
