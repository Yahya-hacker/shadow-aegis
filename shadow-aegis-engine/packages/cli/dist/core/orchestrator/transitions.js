...
import { isTerminalPhase, isValidTransition, } from './mission-state.js';
/**
 * Attempt a state transition with validation.
 */
export function attemptTransition(currentState, targetPhase, reason, context = {}) {
    // Validate transition is allowed
    if (!isValidTransition(currentState.currentPhase, targetPhase)) {
        return err(`Invalid transition: ${currentState.currentPhase} → ${targetPhase}. ` +
 ...
 */
export function getAllowedTransitionsForState(state) {
 ...
 */
export function calculateMissionConfidence(state) {
