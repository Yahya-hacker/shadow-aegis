...
export class PolicyAuditManager extends EventEmitter {
    ...
    generateReport() {
        ...
        const lines = [];
        lines.push('# Policy Audit Report', `Run ID: ${this.runId}`, `Generated: ${new Date().toISOString()}`, '', '## Summary', `- Total Decisions: ${stats.totalDecisions}`, `- Allowed: ${stats.allowed}`, `- Denied: ${stats.denied}`, `- Expert Overrides: ${stats.expertOverrides}`, '', '## By Type');
        ...
        try {
            ...
            for (const line of lines) {
                try {
                    ...
                    console.warn(`[PolicyAuditManager] Skipping invalid audit entry: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
        catch (error) {
     ...
     */
    recordCommandDecision(command, decision, expertOverride = false) {
     ...
     */
    recordConfirmation(entryId, result) {
    ...
    recordMCPDecision(serverName, toolName, toolArgs, decision, expertOverride = false) {
     ...
     */
    // eslint-disable-next-line max-params
    recordPathDecision(requestedPath, resolvedPath, operation, allowed, reason) {
    ...
    createEntry(type, allowed, reason, context) {
        this.entryCount++;
        return {
            allowed,
 ...
 */
export function initializeAuditManager(runId, auditDir) {
 ...
 */
export function getAuditManager() {
    return globalAuditManager;
}
