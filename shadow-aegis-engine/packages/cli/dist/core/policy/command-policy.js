...
export function evaluateCommandPolicy(command, config = {}) {
    ...
    if (matchedAllow) {
        return {
            ...
            ...(matchedDeny
                ? {
                    warning: '[EXPERT-UNSAFE] This command matches a dangerous pattern but expert mode is enabled. Confirm carefully.',
                }
                : {}),
        };
    }
    if (config.expertUnsafe) {
        return {
            ...
            warning: '[EXPERT-UNSAFE] Command is outside the safe allowlist. Proceed only if you fully understand the impact.',
        };
    }
    return {
        allowed: false,
        reason: '[POLICY_DENIED] Command family is not allowlisted. Allowed families: git status|diff|log, npm test|run lint|run build.',
    };
}
