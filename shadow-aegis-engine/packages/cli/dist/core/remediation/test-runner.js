 ...
 * - Go: --- PASS / --- FAIL
 */
function parseTestOutput(stdout, framework) {
    const entries = [];
...
function computeFingerprintHash(entries) {
...
export class TestRunner {
     ...
     */
    static fromFramework(options, framework) {
        return new TestRunner(options, framework);
    }
    /**
    ...
    async run() {
        ...
        if (this.baseline) {
            const baselineFailures = new Set(this.baseline.entries
                .filter((e) => e.status === 'fail')
                .map((e) => e.testName));
            const currentFailures = new Set(entries
                .filter((e) => e.status === 'fail')
                .map((e) => e.testName));
    ...
    buildCommand() {
        if (this.useDocker) {
            ...
            dockerArgs.push('-w', '/app', '--env', 'CI=true', '--memory', '512m', '--cpus', '1', this.containerImage, 'sh', '-c', runCmd);
