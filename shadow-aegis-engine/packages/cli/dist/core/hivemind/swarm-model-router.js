 ...
 */
export function classifyModelTier(provider, model) {
 ...
 */
export function computeTrustScore(tier) {
    switch (tier) {
        case 'local': {
            return 0.5;
        }
 ...
 */
export function resolveWorkerModel(role, defaultModel, overrides) {
    if (!overrides) {
        return defaultModel;
    }
 ...
 */
export function resolveWorkerTier(role, defaultProvider, defaultModelName, overrides) {
    const override = overrides?.[role];
