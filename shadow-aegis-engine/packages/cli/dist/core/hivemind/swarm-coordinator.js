...
import { resolveWorkerModel, resolveWorkerTier, } from './swarm-model-router.js';
/**
...
export class SwarmCoordinator {
    ...
    async executeMission(userMessage, onActivity) {
        ...
        if (!reconRes.ok)
            throw new Error(reconRes.error);
        ...
        if (!taintRes.ok)
            throw new Error(taintRes.error);
        ...
        if (!exploitRes.ok)
            throw new Error(exploitRes.error);
        ...
        // 3. Spawn workers
        const rolesToSpawn = ['recon', 'taint-tracer', 'exploit-analyst', 'verifier', 'reporter'];
        if (patchEnabled) {
        ...
        for (const role of rolesToSpawn) {
            ...
            const overrides = this.config.swarm?.modelOverrides;
            const workerModel = resolveWorkerModel(role, this.model, overrides);
            const { modelTier, trustScore } = resolveWorkerTier(role, this.config.provider, this.config.model, overrides);
        ...
        while (true) {
            ...
            const isFinished = allTasks.every((t) => t.status === 'completed' || t.status === 'failed' || t.status === 'cancelled');
            if (isFinished) {
                break;
            }
            ...
            for (const task of claimable) {
                ...
                const idleWorker = [...this.workers.values()].find((w) => w.role === task.requiredRole &&
