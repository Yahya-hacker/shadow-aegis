...
import { taskSchema, } from './hivemind-schema.js';
/**
 * Manages tasks with dependency tracking.
 */
export class TaskGraph {
    taskDependents = new Map(); // taskId -> tasks that depend on it
    tasks = new Map();
    ...
    importTasks(tasks) {
        this.tasks.clear();
        for (const set of this.tasksByStatus.values())
            set.clear();
    ...
    determineInitialStatus(dependencies) {
    ...
    updateBlockedDependents(completedTaskId) {
    ...
    wouldCreateCycle(newTaskId, dependencies) {
