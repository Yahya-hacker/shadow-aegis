...
import { NullEmbeddingProvider, OllamaEmbeddingProvider, OpenAIEmbeddingProvider, SemanticIndex, } from './memory/semantic-index.js';
import { resolveRuntimeSettings } from './model-capabilities.js';
import { getModel } from './model-router.js';
import { MissionEngine } from './orchestrator/mission-engine.js';
import { computeCiExitCode, formatCiSummary } from './output/ci-exit.js';
...
function maybeCreateHttpInvoker(endpoint) {
...
export class AgentSession {
    ...
    systemPrompt = '';
    tools = {};
    constructor(config, repoMap, targetPath, options = {}) {
        this.config = config;
...
Use your tools to inspect implementation details, verify assumptions, and produce precise security findings.`,
    ...
    async sendMessage(userMessage, onChunk, onEvent) {
...
Return corrected JSON now.`;
                ...
                if (this.config.ci?.enabled) {
                    const failOn = (this.config.ci.failOn ?? 'high');
                ...
                this.runtimeWarnings.push(`Report validation fallback applied: ${error.message}`);
            }
...
Return corrected JSON now.`;
    ...
    async completeMissionCycle(actionId, report, tokensUsed) {
        ...
        await this.transitionMission('REPORT', report.findings.length > 0 ? 'verification_passed' : 'verification_failed', {});
    ...
    async ingestReportFindings(report) {
        ...
        for (const finding of report.findings) {
            const vulnerabilityId = vulnerabilityCanonicalId(finding.cwe, undefined, undefined, finding.title);
            graph.addEntity({
    ...
    async initializeMcpTools(targetPath, enabled) {
        if (!enabled) {
            return {};
        }
        ...
        const chromeInvoker = maybeCreateHttpInvoker(this.config.mcp?.chromeDevtoolsEndpoint ?? process.env.SHADOW_AUDITOR_MCP_CHROME_ENDPOINT);
        const kaliInvoker = maybeCreateHttpInvoker(this.config.mcp?.kaliLinuxEndpoint ?? process.env.SHADOW_AUDITOR_MCP_KALI_ENDPOINT);
    ...
    async initializeMissionRuntime(resolvedTargetPath) {
        if (!this.artifacts) {
            return;
        }
        try {
            ...
            this.runtimeWarnings.push(`Mission engine initialization failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async initializeSemanticIndex(resolvedTargetPath) {
        ...
        try {
            ...
            const { chunksIndexed, filesIndexed } = await this.semanticIndex.indexRepository((progress) => {
                if (progress.filesIndexed % 50 === 0 || progress.filesIndexed === progress.totalFiles) {
                    console.log(`[SemanticIndex] Indexed ${progress.filesIndexed}/${progress.totalFiles} files (${progress.currentFile})`);
                }
            });
            console.log(`[SemanticIndex] Indexing complete: ${filesIndexed} files, ${chunksIndexed} chunks`);
            // Build HybridRetriever if MissionEngine graph is available
            if (this.missionEngine) {
                const graph = this.missionEngine.getGraph();
                const retrieval = this.missionEngine.getRetrieval();
                const hybridRetriever = new HybridRetriever(graph, retrieval, this.semanticIndex, { rootPath: resolvedTargetPath });
            ...
            this.runtimeWarnings.push(`Semantic indexing failed (falling back to repo-map only): ${error instanceof Error ? error.message : String(error)}`);
    ...
    async startMissionCycle(userMessage) {
        if (!this.missionEngine) {
            return null;
        }
    ...
    async transitionMission(targetPhase, reason, context) {
        if (!this.missionEngine) {
            return;
        }
        ...
        if (!result.ok) {
            this.runtimeWarnings.push(`Mission transition ${currentState.currentPhase}→${targetPhase} blocked: ${result.error}`);
        }
    }
}
