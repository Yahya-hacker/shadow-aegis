...
import { z } from 'zod';
export function createContextRetrievalTool(options) {
    const { retriever, rootPath } = options;
    return tool({
        description: 'Searches the codebase using hybrid retrieval (semantic similarity, keyword matching, and knowledge graph traversal). ' +
        ...
        async execute({ fileFilter, maxResults, query, strategy, }) {
            try {
                ...
                return `[ERROR] Context retrieval failed: ${error.message}`;
            }
        },
        inputSchema: z.object({
            ...
            query: z
                .string()
                .min(3)
                .describe('Natural language query describing what code you are looking for. ' +
                'Be specific: "SQL query construction without parameterization" is better than "SQL".'),
            strategy: z
                .enum(['semantic', 'lexical', 'graph', 'hybrid'])
                .optional()
                .describe('Retrieval strategy. "semantic" for meaning-based search, "lexical" for keyword matching, ' +
