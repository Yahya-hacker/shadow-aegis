...
export function createListDirectoryTool(pathGuard) {
    return tool({
        description: 'Lists directory contents using symlink-safe path resolution. Use this to discover files and repository layout.',
        async execute({ path: dirPath }) {
            try {
                ...
                return `[ERROR] Could not list directory "${dirPath}": ${error.message}`;
            }
        },
