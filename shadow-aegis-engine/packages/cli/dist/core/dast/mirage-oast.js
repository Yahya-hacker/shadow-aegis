...
`.trim().replaceAll("'", String.raw `'\''`);
    ...
    async syncLog() {
        if (!this.running)
            return [];
        const result = await this.dockerExec(`docker exec ${this.containerName} wget -qO- http://localhost:8080/__mirage/log`);
        if (result.exitCode !== 0)
            return [];
        try {
            ...
            const existingKeys = new Set(this.callbackLog.map((cb) => `${cb.timestamp}:${cb.url}`));
