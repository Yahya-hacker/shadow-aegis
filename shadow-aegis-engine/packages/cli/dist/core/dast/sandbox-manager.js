...
export class SandboxManager {
    ...
    async create() {
        ...
        else if (await fileExists(path.join(absTargetPath, 'pyproject.toml')) ||
        ...
        createCmd.push('-w', '/app', this.options.baseImage, 'sleep', 'infinity');
    ...
    async deploy() {
        ...
        if (this.options.healthCheckUrl) {
            ...
            for (let i = 0; i < maxAttempts; i++) {
                const healthCheck = await this.exec(`wget -qO- --timeout=5 ${this.options.healthCheckUrl} 2>/dev/null || true`);
    ...
    async exec(command) {
        ...
        const result = await this.dockerExec(`docker exec ${this.containerName} sh -c ${this.shellEscape(command)}`);
        const execResult = {
            command,
    ...
    dockerExec(command) {
        return new Promise((resolve) => {
            exec(command, { maxBuffer: 10 * 1024 * 1024, timeout: this.options.timeoutMs }, (error, stdout, stderr) => {
                resolve({
