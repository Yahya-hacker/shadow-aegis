...
const PR_SCOPE_CHANGED = { H: 0.5, L: 0.68, N: 0.85 };
// ---------------------------------------------------------------------------
// Parsing
// ---------------------------------------------------------------------------
/**
 ...
 */
export function computeCvssBaseScore(metrics) {
 ...
 */
export function cvssScoreToSeverityLabel(score) {
    if (score >= 9)
        return 'Critical';
    if (score >= 7)
        return 'High';
 ...
 */
export function checkCvssConsistency(reportedScore, vector, tolerance = 0.5) {
    const parsed = parseCvssVector(vector);
    if (!parsed.valid || !parsed.metrics) {
        return {
            isConsistent: false,
